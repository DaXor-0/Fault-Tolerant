...
# Build and run the local fault and performance campaigns with 32 MiB caps.
...
echo "==> Running local fault-injection campaign"
(
  cd "$ROOT_DIR/run"
...
echo "==> Running local performance comparison"
(
  cd "$ROOT_DIR/run"
  REPEATS="$REPEATS" \
  MAX_NP="$MAX_NP" \
